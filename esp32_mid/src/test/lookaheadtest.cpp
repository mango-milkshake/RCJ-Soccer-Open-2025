...
void getTopCamData(){
    if(Serial1.available()>=CAM_SERIAL_DATA_LEN){
        ...
        if(len!=CAM_SERIAL_DATA_LEN || uartBufferCam[0]!=5){
            ...
            if(uartBufferCam[5] == 0) top_ball_vy *= -1;
            top_ball_vx = (float)(uartBufferCam[9] + (uartBufferCam[10]<<8)) / 128;
            if(uartBufferCam[8] == 0) top_ball_vx *= -1;
            ...
            // ball_vx += top_ball_dist*self_velocityw*cosf((3.1415/2)-atan2f(top_relative_ball_y,top_relative_ball_x));
...
void frontCamTrack(){ 
    ...
    movement(self_x, self_y, atan2(approx_future_y, approx_future_x));
}
