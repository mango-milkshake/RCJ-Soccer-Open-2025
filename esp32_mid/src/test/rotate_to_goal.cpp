...
// PID
float pid_def_rotate_default[3] = {4, 0, 0};
float pid_def_x_default[3] = {3.5, 0, 0};
float pid_def_y_default[3] = {3.5, 0, 0};

float pid_att_rotate_default[3] = {0.7, 0, 0};
...
int open_rows_start;
int open_rows_end;
int total_rows = 24; //update based on front cam code, given by int(240/y_separation)
int fov = 100; //deg
...
void getMidPlateData(){
    ...
    else if (midRcvBuffer[5] == 0){
        frontPathClear = false;
    }
    open_rows_start = midRcvBuffer[6];
    open_rows_end = midRcvBuffer[7];
...
int goal_heading_right;
int goal_heading_left;
float start_heading;
float end_heading;
void loop(){
    ...
    if(!frontPathClear){
        start_heading = open_rows_start*(fov/total_rows) - fov/2;
        end_heading = open_rows_end*(fov/total_rows) - fov/2; 
        goal_heading_right = DEG(atan2(self.x-OPP_GOAL_RIGHT_X, OPP_GOAL_Y-self.y));
        goal_heading_left = DEG(atan2(self.x-OPP_GOAL_LEFT_X, OPP_GOAL_Y-self.y));
        // DEBUG(self.x)
        // DEBUG(self.y)
        // DEBUG(start_heading);
        // DEBUG(end_heading);
        // DEBUG(goal_heading_right);
        // DEBUG(goal_heading_left); 
        if (start_heading < goal_heading_right){
            start_heading = goal_heading_right;
        }
        if (end_heading > goal_heading_left){
        ...
        Serial.println("Front path clear");
    }
