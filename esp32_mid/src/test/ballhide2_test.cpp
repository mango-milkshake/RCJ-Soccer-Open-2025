...
void sendData(){ //send data here
    ...
    espnowData.ready = ready_to_ballhide;
    espnowData.state = ballhide_state;
...
void ballHide3_1(){ //botID 1 (leader)
    switch (ballhide_state) {
        case 0: //begin
            ballhide_state = 1;
            break;
        case 1: //move to starting point
            movement(1.5, 0.8, 90);
            if (within(self.x, 1.5, 0.05) && within(self.y, 0.8, 0.05)){
                ballhide_state = 2;
            }
            break;
        case 2: //move to goal along wall
            movement(1.5, 1.8, 90);
            if (within(self.x, 1.5, 0.05) && within(self.y, 1.8, 0.05)){
                ballhide_state = 3;
            }
            break;
        case 3: //shoot at goal
            movement(1.2, 1.8, -45);
            break;
        default:
            ballhide_state = 0;
...
void ballHide3_2(){ //botID 2 (decoy)
    switch (ballhide_state){
        case 0: 
            movement(FIELD_WIDTH/2, FIELD_HEIGHT/2, 0);
            if(espnowDataRecv.state == 1){
                ballhide_state = 1;
            }
            break;
        case 1:
            movement(1.5, 0.8, 90);
            if (espnowDataRecv.state == 2 && within(self.x, 1.5, 0.05) && within(self.y, 0.8, 0.05)){
                ballhide_state = 2;
            }
            break;
        case 2:
            movement(1.5, 1.8, 90);
            if (espnowDataRecv.state == 3 && within(self.x, 1.5, 0.05) && within(self.y, 1.8, 0.05)){
                ballhide_state = 3;
            }
            break;       
        case 3:
            movement(1.2, 1.8, -45);
            if (espnowDataRecv.state == 0){
                ballhide_state = 0;
            }
            break;            
        default:
