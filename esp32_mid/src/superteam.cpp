...
void movement(float target_x, float target_y, float target_rotation){
    ...
    target_x = constrain(target_x, 0.50, FIELD_WIDTH - 0.50);
    if(self.x > SELF_GOAL_LEFT_X && self.x < SELF_GOAL_RIGHT_X) target_y = constrain(target_y, 0.80, FIELD_HEIGHT - 0.80);
    else target_y = constrain(target_y, 0.50, FIELD_HEIGHT - 0.50);

    if((self.y <= 0.80 || self.y >= FIELD_WIDTH - 0.80) && 
        ((self.x <= SELF_GOAL_LEFT_X && target_x >= SELF_GOAL_RIGHT_X) || (self.x >= SELF_GOAL_RIGHT_X && target_x <= SELF_GOAL_LEFT_X)))
        target_y = constrain(target_y, 1.00, FIELD_HEIGHT - 1.00);
...
void loop(){
    ...
    if(state.botID == 1 && ball.noBall && !espnowDataRecv.noBall){ 
        // if do not see ball, but other bot does, use other bot's ball pos
        // currently attacker to defender only
    ...
    if(state.botID == 1){
        // defender
        if(ball.ballCap && state.st == 0){
            // if(self.x <= OPP_GOAL_LEFT_X) state.st = 1; // left ballhide
            // else if(self.x >= OPP_GOAL_RIGHT_X) state.st = 2; // right ballhide
            // else if(self.y <= FIELD_HEIGHT/2) state.st = 3; // mid ballhide
            // else state.st = 4; // directly score
            if(self.y >= FIELD_HEIGHT/2) state.st = 4;
            else{
                if(self.x <= FIELD_WIDTH/2) state.st = 1;
                else state.st = 2;
            }
        }
        if(ball.ballCap && (millis() - ball.lastNoBallCap < ball.ballCapTime || drib.speed < drib.reach_speed)){
        ...
        else if(ball.ballCap) {
            if(state.topStrat == 1){
                if(state.st == 1) bot.ballHideLeftOnly();
                else if(state.st == 2) bot.ballHideRightOnly();
                else if(state.st == 3) bot.ballHideMid();
                else bot.staim();
            }
            else bot.staim();
        }
        else if (ball.noBall || (ball.absolute_x > SELF_GOAL_LEFT_X && ball.absolute_x < SELF_GOAL_RIGHT_X && ball.absolute_y < 0.70f))
            bot.moveToPoint(FIELD_WIDTH/2, DEF_Y_DEFAULT, 0);
        else if (ball.absolute_y < DEF_Y_DEFAULT) bot.dribblerBallTrack();
        else bot.defend();
    }
    else{
        // attacker
        ...
        if(ball.ballCap && (millis() - ball.lastNoBallCap < ball.ballCapTime || drib.speed < drib.reach_speed)){
            ...
            state.st = 0;
            #ifdef SUPERTEAM
            bot.oscillateAboutPoint(FIELD_WIDTH/2, FIELD_HEIGHT/2 - 0.20, 1.50);
            #else
            bot.oscillateAboutPoint(FIELD_WIDTH/2, FIELD_HEIGHT/2, 0.70);
            #endif
        }
        else {
            state.st = 0;
    ...
    if(!ball.ballCap && state.botType == 1){
        if(move.y > DEF_Y_MAX){
            move.y = DEF_Y_MAX;
        }
    }
