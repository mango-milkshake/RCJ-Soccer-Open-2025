...
int defCount = 0;
int atkCount = 0;
bool _isDefender; //to check if switched
...
void assignDef(){
    ...
    if(espnowDataRecv.def == true){  //check if other bot is defending
        ...
        if(defCount >= assignDefBuffer){
            Serial.println("switched to attacker: 1");
            isDefender = false;
           // defCount = 0;
        }
    }
    ...
    if(ballCap && millis() - lastNoBallCap >= DEFENDER_WAIT_TIME){ //check if the bot has the ball 
        Serial.println("switched to attacker: 2");
        isDefender = false;
    }
    if(espnowDataRecv.inField == false || otherBotExists == false){ //check if the other bot is in the field
        isDefender = true;
        if (!otherBotExists){Serial.println("other bot gone");}
        if (!espnowDataRecv.inField){Serial.println("other bot not in field");}
    }
    if(isTilted){
        Serial.println("switched to attacker: 3");
        isDefender = false;
    }
    //DEBUG(espnowDataRecv.inField);
    // if(_isDefender != isDefender){
...
void lookAhead(){
    ...
    // }
        ...
        LAball_vy = ball_vy + self_velocityy;
    }

int lookahead_n = 0;
while(!lookAheadConfirm && lookahead_n < 10){ 
    lookahead_n++;
    ...
    targetheadinglookahead = atan2(targetballposx,targetballposy);
...
void setup(){
    ...
    esp_led.setPixelColor(0, esp_led.Color(8, 2, 8));
    esp_led.show();
...
*/
    ...
    else if (!moveToGoal && !noBall && checkv(5, 0.2)){ // if last n values are within x of each other, update look ahead target
    ...
    if(targetballposx<0 || targetballposx>FIELD_WIDTH || targetballposy<0 || targetballposy>FIELD_HEIGHT){
        ...
        movement(targetballposx, targetballposy,  targetheadinglookahead * (180/3.14159));
        // sendI2C(zeroBuffer);
    }
