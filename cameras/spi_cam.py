...
thresh_ball = (31, 80, 11, 50, 5, 79)
thresh_yellow_goal = (60, 88, -26, -2, 12, 60) # (63, 81, -18, 8, 30, 67)
thresh_blue_goal = (41, 61, -28, -12, -28, -11) # (57, 75, -30, 3, -35, -19) # (49, 58, -29, 3, -34, -10)
...
#        print("actual", actual_dist)
        print("angle ", ball_angle, "dist ", actual_dist)
...
#            img.draw_cross(bg.cx(), bg.cy())
    ...
    if(useUART):
        ...
        sendBuffer.append(1)
        if(no_ball==False):
            sendBuffer.append(angle_uart & 0xFF)
            sendBuffer.append((angle_uart >> 8) & 0xFF)
            sendBuffer.append(dist_uart & 0xFF)
            sendBuffer.append((dist_uart >> 8) & 0xFF)
        else:
            sendBuffer.append(0)
            sendBuffer.append(0)
            sendBuffer.append(0)
        ...
        if(no_goal==False):
            if(use_goal=="blue"):
                sendBuffer.append(angle_bgoal_uart & 0xFF)
                sendBuffer.append((angle_bgoal_uart >> 8) & 0xFF)
                sendBuffer.append(dist_bgoal_uart & 0xFF)
                sendBuffer.append((dist_bgoal_uart >> 8) & 0xFF)
            else:
                sendBuffer.append(angle_ygoal_uart & 0xFF)
                sendBuffer.append((angle_ygoal_uart >> 8) & 0xFF)
                sendBuffer.append(dist_ygoal_uart & 0xFF)
                sendBuffer.append((dist_ygoal_uart >> 8) & 0xFF)
        else:
            sendBuffer.append(0)
            sendBuffer.append(0)
            sendBuffer.append(0)
